...
struct BatteryHealthChargingView: View {
    ...
    @AppStorage("maximumCapacity") private var maximumCapacity = 100
    ...
    var body: some View {
        CustomList(title: "BATTERY_HEALTH".localize(table: table)) {
            ...
            // Maximum Capacity - giữ nguyên kiểu chữ mặc định
            Section {
                Button {
        ...
        .sheet(isPresented: $editingCapacity) {
            VStack(spacing: 16) {
                Text("Chỉnh dung lượng tối đa (%)")
                    .font(.headline)
                    .padding(.top)
                Stepper(value: $maximumCapacity, in: 50...100, step: 1) {
